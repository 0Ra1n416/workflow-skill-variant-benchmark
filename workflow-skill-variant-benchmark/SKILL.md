...
3. 校验通过后初始化：
   ...
   - 若返回 `error: session_exists`，问用户是**继续上次的进度**还是**重来**（加 `--force`）。
     注意：旧 session 可能因为配置文件被移动而失效（`status` 会报 `invalid_config`），
     这时只能 `--force` 重来。
   - `init` 成功后，**默认就把用户送到 TUI**（见下「交互方式」），
     而不是自己用 `AskUserQuestion` 一页页问。

### 交互方式：**默认让用户跑 TUI**，不要用 CC 的问答

**默认路径是 TUI** —— 用户在新开的终端里跑 `tui`，有箭头键和空格多选，一屏能看完全部选项。
**不要**默认用 `AskUserQuestion` 一页一页地问：那要十几次点击才能填完 1–7 步，用户会烦。

只有两种情况才改用 CC 内的 JSON 协议：

1. 用户反馈 TUI 用不了（开不了终端、`questionary` 装不上、报错等）；
2. 用户明确要求「就在对话里问」。

#### 默认路径：让用户在新终端里跑 TUI

`check` 和 `init` 由你**先跑完**（见步骤 0），`.wfbm/` 就建好了。然后对用户说 ——
**命令必须写全，不许留 `<>`、`wfbm`、`$SKILL` 之类的占位符**，用户会原样粘贴：

> 请**新开一个终端窗口**（Windows Terminal / PowerShell / SSH 都行），依次执行：
>
> ```text
> cd "<放着 workflow.config.json 的那个项目目录>"
> <这里写 init 的完整命令>          ← 只有第一次需要
> <这里写 tui 的完整命令>
> ```
>
> 答完回到这边跟我说一声「答完了」就行 —— **不用转述你选了什么**，
> 你的选择都已经存进 `.wfbm/session.json`，我直接读。

两点注意（也一并告诉用户）：

- `cd` 到的是**他自己的项目目录**（有 `workflow.config.json` 的那个）。cwd 很关键，`.wfbm/` 就建在那儿。
- 中途想退出按一次 `Ctrl+C`，进度会自动保存。

**两个终端共享 `.wfbm/session.json`**，所以进度可以互相接力：用户在 TUI 里答到一半退出，
你这边能接着走；你在 CC 里答了几页，用户再用 TUI 也能从断点续上。

用户说答完之后：**不要追问他选了什么** —— 直接跑 `status --json`，全部选择都在
`.wfbm/session.json` 里。然后看相位：

- `cancelled: true` → 用户在 TUI 里选了「取消」，到此为止，不要继续后面步骤；
- `phase` 已是 `done` → 直接跳到步骤 8；
- 还没到 `done` → 说明他中途 `Ctrl+C` 退出了，把上面那段话再说一遍请他继续。

#### 回退路径：CC 内的 JSON 协议

用 `next` / `submit` + `AskUserQuestion` 一问一答地驱动。没有箭头键，选项超过 4 个时
退化成编号列表（见下文 `render=numbered`）。
**只在用户明确要求、或 TUI 确实用不了时才走这条，不要自作主张选它。**

---

## 步骤 1–7 · 收集信息

**默认由用户在 TUI 里完成**（见上文「交互方式」）——你只需在他说答完之后跑一次
`status --json` 确认相位，然后进入步骤 8。

下面这套循环是**回退路径**：只在用户明确要求、或 TUI 用不了时才用。
